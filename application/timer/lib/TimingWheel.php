...
class TimingWheel
{
     ...
     * @return array
     */
    public function popSlots($now_time){
        $cur_index = $this->getCurIndex($now_time);
//        print_r('to do'.$cur_index);
     ...
     * 新增一个数据到时间仑片中
     */
    public function add($interval, $data, $now_time){
        $cur_index = $this->getCurIndex($now_time);
        $total_index = $cur_index + $interval;
     ...
     * 获取当前时间轮片已走到哪个节点
     */
    public function getCurIndex($now_timestamp){
